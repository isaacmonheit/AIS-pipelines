...
int main(int argc, char *argv[])
{
    ...
    GstElement *x264enc1 = gst_element_factory_make("x264enc", "encoder1");
    ...
    GstElement *x264enc2 = gst_element_factory_make("x264enc", "encoder2");
    ...
    if (!pipeline1 || !interpipesrc1 || !vidconvert1 || !x264enc1 || !rtph264pay || !udpsink)
    {
      g_printerr("Not all elements in pipeline could be created.\n");
      return -1;
    }
    
    if (!pipeline2 || ! interpipesrc2 || !vidconvert2 || !x264enc2 || !enccaps || !mpegtsmux || !filesink)
    {
    ...
    gst_bin_add_many(GST_BIN(pipeline1), interpipesrc1, vidconvert1, fakesink1, x264enc1, rtph264pay, udpsink, NULL);
    gst_bin_add_many(GST_BIN(pipeline2), interpipesrc2, vidconvert2, fakesink2, x264enc2, mpegtsmux, filesink, NULL);
    ...
    /* Link pipeline 1 */
    if (gst_element_link_many(interpipesrc1, vidconvert1, fakesink1, NULL) != TRUE) {
        g_printerr("interpipesrc1, vidconvert1, and x264enc1 could not be linked.\n");
    ...
    //if (gst_element_link_filtered(x264enc1, rtph264pay, enccaps) != TRUE) {
    //    g_printerr ("x264enc1 and rtph264pay could not be linked.\n");
    ...
    //    g_printerr ("rtph264pay and udpsink could not be linked.\n");
    ...
    /* Link pipeline 2 */
    if (gst_element_link_many(interpipesrc2, vidconvert2, fakesink2, NULL) != TRUE) {
        g_printerr ("interpipesrc2, vidconvert2, and x264enc2 could not be linked.\n");
    ...
    //if (gst_element_link_filtered(x264enc2, mpegtsmux, enccaps) != TRUE) {
    //    g_printerr ("x264enc2 and mpegtsmux could not be linked.\n");
    ...
    std::cerr << "\nALL ELEMENTS SET TO PLAYING\n";
    std::cerr << "GETTING BUSES\n\n";
