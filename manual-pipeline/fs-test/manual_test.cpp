...
int main(int argc, char *argv[])
{
  ...
  /* Signal handler to catch Ctrl-C and gracefully stop pipeline*/
  ...
  GstElement *muxxer = gst_element_factory_make("flvmux", "muxxer");
  ...
  if (!pipeline || !source || !caps || !vidconvert2 || !x264enc || !muxxer || !filesink)
  {
  ...
  g_object_set(filesink, "location", "output.flv", NULL);

  std::cerr << "1\n";

  /* Add elements to the pipeline */
  gst_bin_add_many(GST_BIN(pipeline), source,
                   vidconvert2, x264enc, muxxer, filesink, NULL);

  std::cerr << "2\n";

  /* Link elements */
  if (gst_element_link_filtered(source, vidconvert2, caps) != TRUE) {
  ...
  if (gst_element_link_many(vidconvert2, x264enc, muxxer, filesink, NULL) != TRUE) {
    g_printerr("interpipesrc1 onward could not be linked.\n");
    gst_object_unref (pipeline);
    return -1;
  }
